 ...
 * declared topic has more than one partition the driver wires its multi-partition task graph;
 * declaring only single-partition topics keeps the legacy single-flat-task behaviour. The existing
 * {@link TopologyTestDriver} constructors remain available and behave exactly as before.</p>
 *
 * <pre>{@code
